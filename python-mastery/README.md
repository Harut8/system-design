...
|---|-----|-------------|--------|
| **T0** | **Bare metal** | | |
| 00 | `00-cpu-execution-model.md` | pipelining, superscalar, out-of-order, branch prediction & misprediction cost, speculation, µops, execution ports, IPC, SMT | ⬜ |
...
| **T1** | **Operating system** | | |
| 06 | `06-processes-threads-scheduling.md` | process vs thread vs task, EEVDF/CFS, timeslices, context-switch cost, involuntary preemption, affinity, nice/RT priorities, cgroup v2 CPU quota & throttling | ⬜ |
| 07 | `07-virtual-memory.md` | page tables, faults (minor/major), `mmap`, copy-on-write, demand paging, overcommit, OOM killer, RSS vs VSZ vs PSS vs USS, THP, swap & pressure stall info | ⬜ |
...
| 09 | `09-syscalls-and-io.md` | syscall cost & vDSO, blocking vs non-blocking, `select`/`poll`/`epoll`/`kqueue`, edge vs level triggering, **io_uring**, page cache, `fsync`/durability, zero-copy, `O_DIRECT` | ⬜ |
| 10 | `10-signals-fork-exec.md` | signal delivery & async-signal-safety, `fork()` in a threaded process (the classic deadlock), `exec`, FD inheritance & `CLOEXEC`, zombies/orphans, process groups | ⬜ |
...
| 15 | `15-refcounting-and-ownership.md` | borrowed vs new vs stolen refs, `Py_INCREF/DECREF`, **immortal objects (PEP 683)**, biased reference counting, deferred refcounting, refcount contention as a scaling wall | ⬜ |
...
| 20 | `20-eval-loop.md` | `_PyEval_EvalFrameDefault`, computed gotos, frame objects (PEP 523/558), **PEP 659 adaptive specializing interpreter**, inline caches, quickening, deoptimization | ⬜ |
...
| 28 | `28-asyncio-internals.md` | event loop, selectors, `Task`/`Future`, the coroutine protocol, how `await` compiles, `uvloop`, `TaskGroup`, cancellation semantics, eager task factory | ⬜ |
...
| 32 | `32-profiling.md` | `cProfile`, **py-spy**, Scalene, **memray**, `tracemalloc`, austin, flamegraphs, line profiling, attributing time to native/IO/GC | ⬜ |
...
| 43 | `43-testing-strategy.md` | pytest architecture & fixtures, **property-based testing (Hypothesis)**, mutation testing, concurrency & fuzz testing, coverage semantics, flakiness as a systems problem | ⬜ |
