...
|---|-----|-------------|--------|
| **T0** | **Bare metal** | | |
| 00 | [`00-cpu-execution-model.md`](00-cpu-execution-model.md) | the CPU as a dataflow engine wearing a von Neumann mask; pipeline depth as a bet that branches are predictable (**13 cycles on Firestorm, 16.5 *or* 19–20 on Skylake depending only on µop-cache state**) and a mispredict priced properly — **~100 discarded issue slots, not 13 cycles**; register renaming, and why WAR/WAW cost nothing; **the out-of-order window is not the ROB** — Firestorm's ~330 coalesced retire *groups* × 7 µops give a >1000-instruction window, and Travis Downs' point that some other structure is usually the real limit; **x86 and Apple silicon side by side** (Golden Cove 6-wide/512-ROB/5 ALUs/µop cache/SMT vs Firestorm 8-wide/6 ALUs/no µop cache/no SMT), plus Zen 5's dual-ported fetch; top-down analysis and **why high IPC is not the goal**; **TAGE/ITTAGE** and the intellectual arc that matters most here — **Ertl & Gregg (2003) vs Rohou–Swamy–Seznec (2015), where Seznec's own predictor invalidated the folklore his co-authors' field had built**; speculation beyond branches — memory-dependence prediction, and **Apple's Load Address Predictor (M2+) and Load *Value* Predictor (M3+), features nobody documented until SLAP and FLOP attacked them**; the Spectre mitigation tax and what a retpoline would do to an interpreter; **fixed-width decode as AArch64's clearest structural win**; execution ports — **six integer ALUs on Firestorm but exactly one can issue an indirect branch, which is CPython's dispatch**; SMT and **the machine that doesn't have it** (`hw.physicalcpu == hw.logicalcpu == 11`); context switches priced at **~11,000 cycles / ~85,000 issue slots** with the cold-predictor term nobody accounts for; and then CPython — **the tail-call interpreter's 10–15% was a Clang 19 regression being worked around**, and Elhage's buried result that **disabling computed gotos made CPython 1% *faster* on Clang 18 (332 indirect jumps → 3 under clang19)**, so dispatch replication buys control over the compiler, not prediction; **PEP 659 as branch prediction at the bytecode level** (specialize/guard/deopt/backoff), with the PEP itself conceding only "a small, but useful, fraction is from improved dispatch"; refcounting as the RAW chain renaming cannot dissolve | ✅ **written** |
...
| 08 | [`08-allocators.md`](08-allocators.md) | the four-layer cake (freelists → pymalloc → libc → kernel), `brk` vs `mmap` and the **dynamic mmap threshold that teaches itself to retain memory**, glibc chunks/bins/tcache read from source (**76 tcache bins × 16, not the 64 × 7 everyone quotes**), arenas — and **the arena cap changed from 8× cores to 1× cores in glibc 2.44, which the wiki and man page still get wrong**, the three routes memory takes back to the kernel and why `malloc_trim` is not what you think, fragmentation as three unrelated failures, **54 size classes and 8.3% mean internal waste, measured**, jemalloc's dirty/muzzy decay, tcmalloc's `rseq` per-CPU caches, mimalloc's free-list multi-sharding, **the 512-byte pymalloc cliff measured to the byte: +19.2 ns in one 8-byte step**, **pymalloc is 1.92× faster than the system allocator and 0% smaller**, why PEP 703 needed mimalloc for three reasons and only one is thread-safety, a 9-rung diagnosis ladder | ✅ **written** |
...
| 11 | [`11-ipc-and-shared-memory.md`](11-ipc-and-shared-memory.md) | **there are exactly two families — copy through the kernel, or share a mapping and inherit the whole memory-model problem**; pipes (**capacity measured at 65536, and `PIPE_BUF` at 512 here vs Linux's 4096 — an atomicity budget an eighth the size everyone assumes**), the write-then-read subprocess deadlock as arithmetic on a 64 KiB buffer, `SIGPIPE`→`EPIPE`; UNIX sockets, the three types and **`SOCK_SEQPACKET` present as a constant and `EPROTONOSUPPORT` at runtime — feature-detect by calling**, the 8 KiB socketpair vs the 64 KiB pipe (**so `Pipe(duplex=True)`, the default, has 8× less buffer**); **`SCM_RIGHTS` — the integer doesn't cross, the open file description does (fd 15 → fd 17, measured)**, zero-downtime restart, and the ack byte CPython still sends for a 2012 macOS bug; `multiprocessing` disassembled — `Queue` is a pipe **plus a feeder thread**, so **`q.empty()` is `True` right after `q.put()` about 1 time in 7 (28/200 measured)** and `pickle` runs *before* the write lock by design; POSIX shm (**`SharedMemory(size=1)` costs 16384 bytes here, 4× Linux**) and **gh-82300 — the resource tracker destroys segments a `subprocess` merely attached to**, next to `heap.py`'s `mkstemp`+`unlink`+`mmap` idiom that has none of the problem; **four independent structural reasons a Python object cannot live in shared memory**; futex mechanics and **its total absence here**, `PyMutex`'s one byte + parking lot (`MAX_SPIN_COUNT` 40 free-threaded / 0 with the GIL, 1 ms barging prevention), **`multiprocessing.Lock` is a *named POSIX semaphore*** whose holder crashing hangs the pool forever with no diagnostic, and **the macOS tax read out of the C: no `sem_timedwait`, so timed `acquire()` polls to a 20 ms ceiling, and `qsize()` raises**; `eventfd` vs the self-pipe and why a semaphore can't join an event loop; **Python has no memory model, so a shared-memory ready-flag protocol is correct on x86 by luck and broken on this CPU** | ✅ **written** |
| 12 | [`12-observing-a-process.md`](12-observing-a-process.md) | four mechanisms and four prices (counting → sampling → tracing → introspection) and why knowing which one you hold *is* the skill; `/proc` as the free tier (`schedstat`'s runqueue-latency field, and voluntary vs **non**voluntary context switches as the "am I waiting or being preempted" test); `perf_event_open` and **the multiplexing trap — a ratio built from two ungrouped counters is meaningless**; `perf record`, the overflow interrupt, 99 Hz-not-100; the stack-walking problem (`fp` vs `dwarf` vs `lbr`, LBR's 8/16/32-frame truncation silently corrupting flame graphs, and **the <1% frame-pointer cost that reversed two decades of distro defaults**); **measured: a native profiler sampling CPython 3.14 at 1 kHz — 843 samples, zero Python function names, and the `_PyEval_EvalFrameDefault` wall replaced by per-opcode `_TAIL_CALL_*` frames you can read the bytecode mix off**; the perf trampoline, `perf_jit` and the `perf inject` step everyone forgets; kprobes/uprobes/tracepoints/USDT with their costs side by side; ftrace via `echo`; eBPF's real innovation (aggregate in the kernel, ship the summary) and bpftrace's `-p` caveat; **`strace` at 62× vs `perf trace` at 2.5×** (Gregg) plus the `PTRACE_ATTACH` race that can inject `EINTR` into a healthy production process; flame graphs — why the x-axis is alphabetical and what that forecloses; **on- vs off-CPU: `tar` at 50.8 s wall and 12.7 s CPU, so 75% of the answer is in time no CPU profile can contain** — off-CPU, wakeup, and off-wake graphs as the definitive GIL picture; PEP 768 from the `_debugger_support` struct up (why "zero-overhead" means *reusing the eval breaker*, and why a process blocked in `read()` never reaches a safe point), `pdb -p`, `asyncio pstree`, and **measured: 3.14.6 ships `RemoteUnwinder` but not Tachyon**; **measured negative results — SIP refuses DTrace even on your own child, and Darwin's Mach task port denies `RemoteUnwinder` a same-user process**; `perf_event_paranoid`'s four levels, `CAP_PERFMON` over `CAP_SYS_ADMIN`, and the container gate table; USE extended to the GIL and the connection pool; RED and its three implementation errors (**errors are fast, so dropping them makes a dashboard *improve* during an incident**); cgroup throttling diagnosed in one file read; a six-phase observation session | ✅ **written** |
...
| 35 | [`35-memory-optimization.md`](35-memory-optimization.md) | **peak RSS is set by stage overlap, not data size — same pipeline, same output, 2,095.9 MB vs 0.1 MB measured**; the eight-rung ladder ordered by effect size (`__slots__` is rung 4, worth 23% against a plain class); **string dedup 0.79×, objects → packed buffer 0.09×**; **`tracemalloc` costs 2.58× RSS and 10.3× time, and frame depth is free**; `mmap` doesn't lower RSS for a full sweep (512.1 vs 512.2 MB) — it makes RSS *reclaimable and shared*; the `bytes(m)` trap doubles it; **fork child privatises 93.6% of a tuple parent vs 12.7% of an array parent — 16.3× per worker**, and `gc.freeze()` fixes only the GC-traversal half; fragmentation's rising floor measured; container budgets, `memory.events`, exit 137 *(cited)*; **free-threaded +23.9% per object that buys away per-worker duplication** | ✅ **written** |
...
*Next: Tier 0 is now three-quarters written — `04-binary-abi-and-linking.md` and
`05-representation.md` close it out. The manifest in §2 is the build order; the checklists
per tier are the definition of done.*
