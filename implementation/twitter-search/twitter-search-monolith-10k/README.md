# Twitter Search Monolith (10K Scale)
...
**Target Scale**: ~10,000 tweets/day, ~100 concurrent users
...
This monolith is designed for **~10K tweets/day**. When you outgrow it:

1. **Move to 100K scale** when daily tweets exceed 10K consistently
2. **Add connection pooling** when database connections become a bottleneck
3. **Increase Redis memory** when cache hit rate drops below 80%

See `twitter-search-monolith-100k` for the next scale tier.
